...
if (configFileExists) {
  console.warn(
    'WARNING: Prettier config (.prettierrc.js) already exists. Doing nothing.'
  );
  process.exit(0);
}
...
const doesPackageJsonConfigExist = () => {
  ...
  return pkg && pkg.prettier ? true : false;
};
...
if (alternativeConfigFileExists) {
  ...
  console.warn(
    'Please delete all config files other than .prettierrc.js to avoid conflicts.'
  );
}
